...
class EloquentWorkspaceMemberRepository implements WorkspaceMemberRepository
{
    ...
    public function save(WorkspaceMember $workspaceMember): void
    {
        $data = [
            ...
            'joined_at' => $workspaceMember->getJoinedAt()?->format('Y-m-d H:i:s'),
            'updated_at' => now(),
        ];
    ...
    {
        return (int) DB::table('workspace_members')
            ->where('workspace_id', $workspaceId)
            ->count();
    }
    ...
    /**
     * Check if a workspace member exists for a user and workspace.
     */
    public function exists(string $userId, string $workspaceId): bool
    {
        return (bool) DB::table('workspace_members')
            ->where('user_id', $userId)
            ->where('workspace_id', $workspaceId)
            ->exists();
    }
    ...
    private function modelToEntity(array $row): WorkspaceMember
    {
        ...
        $workspaceMember->setId(Uuid::fromString($row['id']));
